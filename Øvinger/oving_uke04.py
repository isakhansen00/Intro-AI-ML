...
def insertion_sort(unsorted_list, verbose = False):
    ...
    if unsorted_list:                       # Non-empty list
        sorted_list = [unsorted_list[0]]    # Insert single element in sorted list
        n_sorted = 1                        # Counter for length of sorted list
        for item in unsorted_list[1:]:      # Iterate over remaining unsorted items
            i = 0                           # Start comparison at index 0
            while i < n_sorted and sorted_list[i] < item:   
                i += 1                      # Increase counter until we find a larger number, or reach end of list
    ...
    else: return unsorted_list              # If empty, return without modification
...
def insertion_sort_inplace(unsorted_arr, verbose = False):
    ...
    arr = copy.copy(unsorted_arr)        # Create a copy (don't modify original list)
    ...
    for i in range(1,len(arr)):           # Iterate from 1, index 0 contains single sorted item
        ...
        while j > 0 and item < arr[j-1]:  # Continue iterating as long as item < list content at current index
        ...
        if verbose:                       # (Optional:) Print progress updates
            ...
            print('[Sorted] [Unsorted]: ' + str(arr[:(i+1)]) + ' ' + str(arr[i+1:]))
...
def merge_sort(unsorted_list):
    ...
    if unsorted_list:               # If non-empty list
        ...
        if N == 1:
            ...
            right = merge_sort(unsorted_list[middle:]) # Right half, e.g. [3:7]
...
def merge(left, right, preserve_originals = False):
    ...
    while left and right:                       # While both lists not empty
        if left[0] < right[0]:                  # Compare first elements of sublists
            merged_list.append(left.pop(0))     # If left[0] smallest, remove left[0] and add to merged list
        else:                                   # If right smallest,
            merged_list.append(right.pop(0))    # If right[0] smallest, remove right[0] and add to merged list
    if left:                                    # If elements remaining in left
        merged_list += left                     # Add (already sorted) remainder of left
    else:                                       # If elements remaining in right
        merged_list += right                    # Add (already sorted) remainder of right
